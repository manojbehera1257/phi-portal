...
export default function App() {
  ...
  async function loadMember(authUser) {
    // Use RPC to find member — avoids custom schema PostgREST limitation
    const { data, error } = await supabase
      .rpc('phi_get_member_by_email', { p_email: authUser.email })

    if (error) {
      console.error('loadMember error:', error.message)
      setAuthLoading(false)
      return
    }

    const memberData = data?.[0] ?? null
